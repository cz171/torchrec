...
def shard(
    ...
    if isinstance(plan, ParameterShardingGenerator):
        gen = plan
        plan = {}
        for table_name, param in shardable_parameters.items():
            plan[table_name] = gen(
                param, get_local_size(env.world_size), env.world_size, device.type, sharder
            )
    else:
        
        for table_name, sharding in plan.items():
            if isinstance(sharding, ParameterShardingGenerator):
                param = shardable_parameters[table_name]
                plan[table_name] = sharding(
                    param, get_local_size(env.world_size), env.world_size, device.type, sharder
                )

    return sharder[type(module)].shard(module, plan, env, device)
